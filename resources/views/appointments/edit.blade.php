            ...
            <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                <div class="flex items-center">
                    ...
                    <h1 class="ml-2 text-2xl font-medium text-gray-900">
                        Edit Appointment: {{ $appointment->patient->name }}
                    </h1>
                </div>
            </div>
                ...
                <form method="POST" action="{{ route('appointments.update', $appointment) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-input id="appointment_date" class="block mt-1 w-full" type="datetime-local" name="appointment_date" :value="old('appointment_date', $appointment->appointment_date->format('Y-m-d\TH:i'))" required />
                    ...
                    <div class="mt-6">
                        ...
                        <textarea id="reason" name="reason" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('reason', $appointment->reason) }}</textarea>
                        <x-input-error :messages="$errors->get('reason')" class="mt-2" />
                    </div>

                    <div class="mt-6">
                        <x-label for="notes" value="{{ __('Additional Notes') }}" />
                        <textarea id="notes" name="notes" rows="4" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes', $appointment->notes) }}</textarea>
